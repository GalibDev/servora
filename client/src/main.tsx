...
function App() {
  ...
  return <>
    ...
    <footer className="footer footer-rich"><div className="footer-about"><button className="brand brand-button" onClick={goHome}><span>S</span>servora</button><p>Trusted local help for your home, wellness and everyday life.</p></div><div className="footer-links"><strong>Explore</strong><button onClick={goHome}>Home</button><button onClick={() => openServices()}>All services</button><button onClick={() => goTo('how')}>How it works</button></div><div className="footer-contact"><strong>Contact</strong><a href="mailto:mirza.galib.palash@gmail.com">mirza.galib.palash@gmail.com</a><span>Project support &amp; inquiries</span></div><small>© 2026 Servora. Thoughtful help, close to home.</small></footer>
    {authOpen && <div className="modal"><form onSubmit={auth}><button type="button" className="close" onClick={() => setAuthOpen(false)}><X /></button><small>WELCOME TO SERVORA</small><h2>{mode === 'login' ? 'Good to see you again.' : 'Create your account.'}</h2>{mode === 'register' && <><label>Name<input name="name" required minLength={2} /></label><fieldset className="role-choice"><legend>I want to join as</legend><label><input type="radio" name="role" value="CUSTOMER" defaultChecked /><span><strong>Customer</strong><small>Book trusted services for your needs</small></span></label><label><input type="radio" name="role" value="PROVIDER" /><span><strong>Service provider</strong><small>List services and manage bookings</small></span></label></fieldset></>}<label>Email<input name="email" type="email" required /></label><label>Password<input name="password" type="password" required minLength={8} /></label>{authError && <div className="form-error">{authError}</div>}<button className="primary" disabled={authLoading}>{authLoading ? 'Please wait…' : mode === 'login' ? 'Log in' : 'Create account'} {!authLoading && <ArrowRight size={17} />}</button><p>{mode === 'login' ? 'New here? ' : 'Already a member? '}<button type="button" className="link" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setAuthError(''); }}>{mode === 'login' ? 'Create an account' : 'Log in'}</button></p></form></div>}
