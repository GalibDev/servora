...
function App() {
  ...
  useEffect(() => {
    Promise.all([api<Service[]>('/services?page=1&limit=3'), api<Category[]>('/categories?page=1&limit=20')])
  ...
  return <>
    ...
    {view === 'home' ? <main>
      <section id="home" className="hero"><div className="eyebrow"><Sparkles size={15} /> Exceptional help, thoughtfully delivered</div><h1>Life runs better<br />with a little <em>help.</em></h1><p>Trusted local professionals for the things that matter—beautifully simple to book, reassuring from start to finish.</p><form className="search" onSubmit={submitSearch}><Search /><input name="search" placeholder="What can we help with?" /><button>Explore services <ArrowRight size={18} /></button></form><div className="proof"><span><Check /> Vetted professionals</span><span><ShieldCheck /> Satisfaction guaranteed</span><span><Star /> Loved by 12,000+ homes</span></div></section>
      ...
      <section id="featured" className="services"><div className="section-title"><div><small>CURATED FOR YOU</small><h2>Services people love</h2></div><button className="link" onClick={() => openServices()}>Browse all <ArrowRight size={16} /></button></div>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}><X size={16} /></button></div>}<div className="cards">{featured.map(service => <ServiceCard key={service.id} service={service} onBook={chooseService} />)}</div></section>
