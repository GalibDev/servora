...
export default function Dashboard({ user, onClose }: { user: DashboardUser; onClose: () => void }) {
  ...
  return <div className="dashboard-page">
    <header className="dash-header"><button onClick={onClose}><ArrowLeft size={18}/> Back to Servora</button><div><LayoutDashboard size={19}/><strong>{user.role === 'CUSTOMER' ? 'My dashboard' : user.role === 'PROVIDER' ? 'Provider studio' : 'Admin console'}</strong></div><span>{user.name}</span></header>
