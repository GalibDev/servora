...
export const categoryCreateSchema = z.object({ name: z.string().trim().min(2).max(80), slug: z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), icon: z.string().max(12).optional(), description: z.string().trim().max(500).optional(), status: z.enum(['ACTIVE', 'INACTIVE']).default('ACTIVE') });
...
export const userUpdateSchema = z.object({ name: z.string().trim().min(2).max(80).optional(), avatar: z.url().optional().or(z.literal('')), role: z.enum(['CUSTOMER', 'PROVIDER', 'ADMIN']).optional(), status: z.enum(['ACTIVE', 'SUSPENDED']).optional() }).refine(value => Object.keys(value).length > 0, 'At least one field is required');
